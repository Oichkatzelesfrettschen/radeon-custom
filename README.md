...
| 0060 SIGBUS isolation gate fires on a re-faulted parked VRAM mapping | hardware-demonstrated; a targeted RS482 refault fire returned SIGBUS on one SIGUSR1 re-touch of the park-zapped mapping (si_code BUS_ADRERR, breadcrumb `parked: SIGBUS on VRAM mmap fault`, boot_id stable), retained as steinmarder-r300 bundle `rs480_sigbus_refault_fire_rs482_20260803T030621Z`; the fire ran on the dev-profile module (srcversion D49F542C, mutate-dev), and the 0060 gate source is identical to the prod module while park is reachable only through the armed dev reset node, so this is the only obtainable evidence |
