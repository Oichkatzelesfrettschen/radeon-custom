...
| Radeon DKMS package 0.6-1 pins the signed `radeon-unified-0.6-profiled-source` checkpoint (tag object 7a011a561c38258180e1f3083a0e5d8e74f5c1dd, driver tree 84b3c5c0282bf37236f2c4fda80eb17048bdd1ed) and carries the parked-device entry contract, under which `radeon_gem_object_create`, `radeon_gem_prime_import_sg_table`, and `radeon_gem_wait_idle_ioctl` each return -EIO once `gpu_parked` latches, before the allocation or the HDP MMIO flush they precede | compile-verified on the 6.18 LTS and 7.1 kernel lines; `check_parked_admission_guards.py` proves the three refusals against known-bad mutations and its own fixtures; no 0.6-1 artifact is produced, signed, installed, or promoted, so the signed 0.5-1 set remains the installed production authority and the signed 0.4-3 set the rollback authority. The intermediate 0.5-2 pin (source commit 5df0aec3b7ad96b78c4f8cbb9bc4096571f8ebc2) merged as a repository state and is superseded before artifact production; it named an unsigned tag object, which `check_radeon_source_pin.py` now rejects |
