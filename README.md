...
| 0060 SIGBUS isolation gate fires on a re-faulted parked VRAM mapping | hardware-demonstrated; a single-factor RS482 refault fire returned SIGBUS on one SIGUSR1 re-touch of the park-zapped mapping (si_code BUS_ADRERR, breadcrumb `parked: SIGBUS on VRAM mmap fault`, boot_id stable), retained as steinmarder-r300 bundle `rs480_sigbus_refault_fire_rs482_20260803T030621Z` |
| Fresh post-park client GEM admission fails closed | defect located and RCA-retained; the create-path refusal `radeon_gem_object_create` returns -EIO under `gpu_parked` landed upstream in linux-radeon-gororoba (commit `ca70647`), absent from the pinned 0.5 checkpoint tree so no package here carries it; supply-side only, sufficiency for the pre-park GTT-mapping path unproven, and the parked device admits a fresh `open` while allocation/mmap behavior is not run |
