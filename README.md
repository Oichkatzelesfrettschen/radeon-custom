...
version axes remain distinct. Version 0.8.17-1 is the active package recipe,
...
0.8.17-1 row is the current recipe record.
...
| Radeon DKMS package recipe 0.8.3-1 pins `radeon-unified-0.8.3-profiled-source` at source commit `74cc62c478bd11778ae398d6d1e5f5f43a12fa7a` and driver tree `4d5217ea558320801dde0ef8ce2d8ffdb7a21ba9` | the pin carries the ATOM and COMBIOS bounds hardening and the FLOAT_2 XY01 synthesized-lane width validator. The production profile compiles against 7.1.8-1-cachyos. Target installation, reboot, loaded-module identity, and the attended tuple run are not run for this package record. |
| Radeon DKMS package recipe 0.8.4-1 pins `radeon-unified-0.8.4-profiled-source` at source commit `ca6bad052ceb718ce0814c020f71f01c710dbe0e` and driver tree `cf89588ab90a763e30cd00b9cfcdf9b7291fe660` | the pin carries the one-shot `radeon_rs480_cp_status` RBBM and CP_STAT paired reader and the parallel Kbuild module build. The production profile compiles against 7.1.8-1-cachyos. Target installation, reboot, loaded-module identity, and the attended CP_STAT observation are not run. |
| Radeon DKMS package recipe 0.8.5-1 pins `radeon-unified-0.8.5-profiled-source` at source commit `cc91fbcccfca4478e5f8f1b505a52da74c7af6fe` and driver tree `dc473f6f0010805a5a5ecb0ad9445417f43f72b8` | the pin carries ABI minor 1 of `radeon_rs480_paired_status_census`, including gate-before-token refusal, bounded published length, abort status, truthful BB CP_FIRST, and nonseekable transport. The production profile compiles against 7.1.8-1-cachyos. Target installation, reboot, loaded-module identity, and attended census cells are not run. |
| Radeon DKMS package recipe 0.8.6-1 pins `radeon-unified-0.8.6-profiled-source` at source commit `e03c1d27ba93caae59165cc61726bdca479dc402` and driver tree `373a2e54129588df7b477519dd2f28370e4ecee5` | the pin carries the token-armed `radeon_rs480_vap_status_census` transport with one verified forced-clock lease, bounded reads, ordered restoration, and restored-readback verification. The production, probe-dev, and mutate-dev profiles compile against 7.1.8-1-cachyos. Target installation, reboot, loaded-module identity, and attended VAP census cells are not run. |
| Radeon DKMS package 0.8.7-1 pins `radeon-unified-0.8.7-profiled-source` at source commit `e2528ea1a0a90f618e699fa62b4b925e06045233` and driver tree `d670e3e85d162ae1a183f37deb3bae2c0d644d9d` | the pin narrows the VAP census force gate to FORCE_VAP and carries the disarmed `radeon_rs480_pll_write_probe`. All three profiles compile against 7.1.8-1-cachyos. The historical package record reports boot `75950e1f`, complete VAP censuses, and exact FORCE_VIP restoration. This repository names no retained target bundle for that run, so the hardware claims remain unpromoted. |
| Radeon DKMS package 0.8.8-1 pins `radeon-unified-0.8.8-profiled-source` at source commit `2e704981b744fba5cbece0d5857c214832738326` and driver tree `7996162539e0b1f314edcf77e8c01a5b54c0750c` | the pin carries `radeon_rs480_vap_status_burst_census` transport `rs482-vap-burst-census/1`. The historical package record reports boot `039f3528`, loaded srcversion `F191B0B0349A3B1BA2B3DF3`, a stable disarmed parameter, and eight byte-exact burst legs. This repository names no retained target bundle for that run, so the target record remains unpromoted pending the bundle join. |
| Radeon DKMS package recipe 0.8.9-1 pins `radeon-unified-0.8.9-profiled-source` at source commit `164167950d0f749468474536dd76045fddedc8b7` and driver tree `a50c8ce2bc6c7645c7fe658470f67dbe2e93c31f` | the pin carries separate one-shot token gates for RB3D cache register 0x4E4C and ZB cache register 0x4F18, a per-module cohort latch, GUI_ACTIVE refusal, and source-policy denominators of 24 features, 28 parameters, and 38 debugfs files. The production, probe-dev, and mutate-dev profiles compile against 7.1.8-1-cachyos. Target installation, reboot, loaded-module identity, and an armed cache read are not run. |
| Radeon DKMS package recipe 0.8.9-2 pins `radeon-unified-0.8.9-profiled-source` at source commit `164167950d0f749468474536dd76045fddedc8b7` and driver tree `a50c8ce2bc6c7645c7fe658470f67dbe2e93c31f` | this packaging revision verifies both signed source tags against the package allowlist, restricts target transport to one production archive, moves optional lifecycle evidence outside that transport, and refuses hazardous hardware runners from the package repository. Source and package checks establish offline identity and construction only. Target installation, reboot, loaded-module identity, cache reads, reset operations, and silicon verdicts are not run. |
| Radeon DKMS package recipe 0.8.10-1 pins `radeon-unified-0.8.10-profiled-source` at source commit `b47a32239d299fadb0c4f835453ff381408ea479`, driver tree `bcb23bff436c0b3fa21771fec7614d922473bb61` | the pin carries two RS48X mechanisms. CP microengine restore containment keeps CP_CSQ_CNTL disabled and publishes the RS4xx parked state before returning -EIO, latches terminal admission with pending CPU-only publication so the active hardware transaction queues cleanup from its final release, and rejects embedded NUL bytes before parsing a CP ME command. GEM handle placement carries a per-device monotonic buffer object identity through creation, GEM_BUSY trace events, and `radeon_gem_info` without changing UAPI. Source-policy denominators hold at 36 build features across prod 12, observe-dev 4, probe-dev 9, and mutate-dev 11. Target installation, reboot, loaded-module identity, armed reads, reset operations, and silicon verdicts are not run for this row. |
| Radeon DKMS package recipe 0.8.11-1 pins `radeon-unified-0.8.11-profiled-source` at source commit `3c5ccb3cfb684c975efbb30c3e312c310b741cf9` and driver tree `e3a54399a004c714402b5c9bd56e1edcfd1caa1c` | the pin adds two RS48X mechanisms to 0.8.10. GTT aperture repack answers a GTT allocation that finds no hole wide enough: `radeon_gtt_compact` evacuates every unpinned idle GTT buffer object to the system domain and re-admits them largest first, so the free index space collects into one extent, and `radeon_gem_object_create` retries the request once after a successful repack. The pinned source locations are `linux-radeon-gororoba:drivers/gpu/drm/radeon/radeon_object.c:465-560` for `radeon_gtt_compact` and `linux-radeon-gororoba:drivers/gpu/drm/radeon/radeon_gem.c:157-232` for `radeon_gem_object_create`; reproduce the lookup with `(git -C ../linux-radeon-gororoba grep -n radeon_gtt_compact 3c5ccb3cfb684c975efbb30c3e312c310b741cf9)` and `(git -C ../linux-radeon-gororoba grep -n radeon_gem_object_create 3c5ccb3cfb684c975efbb30c3e312c310b741cf9)`. The cursor path locks both cursor buffer objects without reserving fence slots, because the pin and unpin it performs emit no command stream work. Source-policy denominators hold at 36 build features across prod 12, observe-dev 4, probe-dev 9, and mutate-dev 11. The development package and the board policy are installed on the RS482 target and runtime-accepted across a reboot: the loaded module reports `gororoba_source_commit` `3c5ccb3cfb684c975efbb30c3e312c310b741cf9`, driver tree `e3a54399a004c714402b5c9bd56e1edcfd1caa1c`, and profile `mutate-dev`, the regenerated initramfs images match the digests the boot entries pin, and the ring test, the IB test, and direct rendering all succeed at GTT 512M and VRAM 128M. The run is retained in steinmarder-r300 as sealed bundle `cachyos-vostro1000-rs482-radeon-unified-0.8.11-1-deployment-runtime`, whose checker joins the loaded module to the package pin, every served kernel to a built module, and every boot entry to the digest of the image it names. Reset operations and silicon verdicts are not run for this row. |
| Radeon DKMS package recipe 0.8.12-1 pins `radeon-unified-0.8.12-profiled-source` at source commit `0104ede3f1964cc844f9f1839cb6953e2639c4e6` and driver tree `e05ddca9ca088bada5e7e17ab59f511ef322630b` | the pin adds two RS48X mechanisms to 0.8.11. The TCL-bypass vertex width check admits COLOR0: `r300_tcl_bypass_vtx_check_reason` widens its admitted `VAP_OUTPUT_VTX_FMT_0` scope to `POS_PRESENT|COLOR_PRESENT` and raises the width floor to 4 dwords for position plus 4 more when `COLOR_PRESENT` is set, ahead of the texcoord component-count sum, while every other FMT0 bit declines under the renamed `R300_TCL_BYPASS_DECLINE_FMT0_BEYOND_MODELED` reason; the retained RS482 direct GA Flat capture on 1002:5974 anchors that leg, and the standalone calibration harness covers 38 cases including a COLOR_0 width-miss known-bad. GTT placement segregates by size: `radeon_ttm_placement_from_domain` requests top-of-aperture insertion for objects at or below 512 KiB, so small objects pack against the top, wide requests cut from the bottom, and the aperture middle keeps its holes wide. Source-policy denominators hold at 36 build features across prod 12, observe-dev 4, probe-dev 9, and mutate-dev 11. The target carried this pin at loaded module srcversion `729892A3F3530EB12B8D842` with the prod profile and `lockup_timeout=0`, until 0.8.13-1 superseded it as the installed authority; reset operations and silicon verdicts are not run for this row. |
| Radeon DKMS package recipe 0.8.13-1 pins `radeon-unified-0.8.13-profiled-source` (tag object `d6e88ebe5acc6b1c7b57e2d1951d88b23495cf81`) at source commit `2be21eaa892723f1c9cd826b7331c7d234e2c1ce` and driver tree `644b64d932603143b1abc719adf1759569f580e8` | the pin adds one RS48X mechanism to 0.8.12. GTT compaction runs reservation-safe: `radeon_gtt_compact` collects GEM references under the object-list mutex, admits movable objects under reservation, sorts candidates through the kernel list primitive, and rechecks each successfully evacuated object before readmission, so a concurrent hold no longer races the sweep; the compaction report separates candidates, skipped, evacuated, readmitted, and stranded objects while the parked and fenced refusal boundaries hold unchanged. The driver subtree is the only source change against 0.8.12: the policy tree and feature digest are byte-identical, source-policy denominators hold at 36 build features across prod 12, observe-dev 4, probe-dev 9, and mutate-dev 11, the archive entry count holds at 222, and the upstream base is unmoved. The COLOR0 TCL-bypass width admission carries forward from 0.8.12. The target carries this pin: the signed production and board-policy archives install in one pacman transaction, DKMS builds `radeon-unified/0.8.13` against 7.1.8-1-cachyos and 6.18.42-1-cachyos-lts at module srcversion `46C05689F2C98A526C314F4`, and the host loads that srcversion across a reboot with the prod profile and `lockup_timeout=0` held from the package-owned board policy. Radeon binds `0000:01:05.0` as `RS480 0x1002:0x5974 0x1028:0x022A`, reports 128M VRAM and 512M GTT with GART enabled, and passes the ring test and the IB test; the command-stream log carries no validation rejection, and the R3V ICD enumerates the device as `ATI RS480 (RS482/RS485)`. Command-stream admission is byte-identical to 0.8.12, which preserves every retained cell verdict without replay: `r100.c`, `r300.c`, `radeon_cs.c`, `rs400.c`, `replay_r300_cs_track.c`, and `replay_r300_tcl_bypass_ib.c` carry the same blob hashes across the two pins, and `radeon_gtt_compact` reaches the driver only through the single `-ENOMEM` GTT retry in `radeon_gem_object_create`, which a single-triangle cell on an unexhausted aperture never enters. The pre-silicon RB2D fill qualification against this module is sealed as steinmarder-r300 bundle `r3v-native-rb2d-const-fill-public-route-prediction-vostro1000_rs485m_5974`, whose kernel replay recorded that `r100_cs_track` tracks no legacy 2D destination, so every 2D destination geometry mutation passed this parser unchecked; 0.8.14-1 supersedes it as the recipe authority for that reason. Reset operations and silicon verdicts are not run for this row. |
| Radeon DKMS package recipe 0.8.14-1 records the preceding package authority at source commit `b534d1b0a90529988f20f1bf4f29648e49d10e29`, driver tree `a11b837b591dba78884afe7744762f878ebb6ff1`, and signed tag `radeon-unified-0.8.14-profiled-source` (tag object `8c8404c1909225ef854f71e81c750321c912ce9a`) | the pin adds one RS48X mechanism to 0.8.13. The command-stream tracker bounds the legacy 2D destination: `r100_reloc_pitch_offset_ex` hands the object the `DST_PITCH_OFFSET` relocation consumed to the caller, `r100_cs_track_2d_dst_bind` records that object with `radeon_bo_size` of it and the decoded 64-byte-granular pitch and 1 KiB-granular offset, `DP_GUI_MASTER_CNTL` fixes the bytes per pixel from the destination datatype and whether the destination comes from the relocated register, `DST_Y_X` records the origin, and both launch registers `DST_WIDTH_HEIGHT` (width high) and `DST_HEIGHT_WIDTH` (height high, the X EXA launch) run `r100_cs_track_2d_dst_check`, which refuses a launch before those three registers, an unsized datatype, an empty rectangle, a zero pitch, an x or x plus width past the pitch, any overflow of the checked u32 surface arithmetic, and an end byte past the object; the whole rectangle is bounded, so hardware clipping never rescues an out-of-object rectangle, and `DST_LINE_START`/`DST_LINE_END` reject until the tracker models a line. The five registers leave every family's `reg_srcs` skip list and the r100, r200, and r300 packet0 checks carry the cases. The driver subtree is the only source change against 0.8.13: the policy tree and feature digest are byte-identical, the archive entry count holds at 222, and the upstream base is unmoved. Source-side calibration: `run_r300_cs_grammar_correspondence.sh` holds at 182 controls with 140 case labels agreeing across two extractions, `run_r300_cs_2d_dst_controls.sh` assembles the 38-dword RB2D fill stream from raw words byte-identical to the retained ib.bin and holds every containment arm with the completion-object arm bound by trace to entry 1 at 4 bytes, and the module builds clean against 7.1.8-1-cachyos and 6.18.42-1-cachyos-lts. Target deployment, srcversion, ring and IB startup, and the RB2D fill re-qualification are not run for this row until the package is installed on the Vostro 1000. |
...
| Radeon DKMS package recipe 0.8.17-1 pins the signed `radeon-unified-0.8.17-profiled-source` checkpoint at source commit `434d5a7259aded6bbd744fe89de37104c5da0355`, with tag object `a6ee356973a812020bab602295155d639164c77f` and driver tree `0a8c576317940720ee836107e0171005565ebdfe` | the pin carries the datatype-9 legacy 2D destination refusal. Source pin, profile, checksum, and package construction checks qualify the packaging. Installation and target execution remain separate evidence gates. |
...
```bash
# 6 known-bad inputs rejected, 4 known-good inputs cleared
